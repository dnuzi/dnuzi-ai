...
const { DnuziAI }   = require("../lib/index.cjs");
...
// ─── conf ──────────────────────────────────────────────────────────────────
function getConf() {
  return new Conf({ projectName: "dnuzi", defaults: { userId: "cli_user", useMongo: false } });
}
...
function printBanner() {
  ...
  const art = figlet.textSync("DNUZI", {
    font: "ANSI Shadow",
...
async function repl() {
  const conf   = getConf();
  const ai     = new DnuziAI({ userId: conf.get("userId") });
  let mongoOn  = false;
  ...
  rl.on("line", async (raw) => {
    ...
    if (input === "/version") {
      console.log("\n  " + C.secondary(S.info) + "  dnuzi " + C.primary("v" + pkg.version) + "\n");
...
if (args.length > 0) {
  // one-shot:  dnuzi "what is gravity?"
